                    ...
                    *f << ")\n";
                    for (int i = 0; i < wire->width; i++) {
                        ...
                        net_join_db[sig].insert(
                          make_pair(stringf("(portRef (member %s %d))", EDIF_REF(wire->name), lsbidx ? i : GetSize(wire) - i - 1), wire->port_input));
                    }
                }
            }
                        ...
                        if (sig[i].wire == NULL && sig[i] != RTLIL::State::S0 && sig[i] != RTLIL::State::S1)
                            ...
                            if (width == 1)
                                ...
                                net_join_db[sig[i]].insert(
                                  make_pair(stringf("(portRef (member %s %d) (instanceRef %s))", EDIF_REF(p.first), member_idx, EDIF_REF(cell->name)),
                                            cell->output(p.first)));
                            }
                        }
                }
