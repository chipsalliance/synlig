...
// Sets the `wire_node->dimensions` attribute and returns the total sizes of packed and unpacked ranges.
static std::pair<size_t, size_t> set_dimensions(AST::AstNode *wire_node, const std::vector<AST::AstNode *> packed_ranges,
                                                const std::vector<AST::AstNode *> unpacked_ranges)
{
    // node->dimensions stores dimensions' offsets and widths.
    ...
    auto calc_range_size = [wire_node](const std::vector<AST::AstNode *> &ranges) -> size_t {
        ...
        for (size_t i = 0; i < ranges.size(); i++) {
            ...
            const int low = min(ranges[i]->children[0]->integer, ranges[i]->children[1]->integer);
            const int high = max(ranges[i]->children[0]->integer, ranges[i]->children[1]->integer);
            const int elem_size = high - low + 1;

            wire_node->dimensions.push_back({low, elem_size, ranges[i]->range_swapped});
            size *= elem_size;
        }
        return size;
    };
...
static AST::AstNode *convert_range(AST::AstNode *id, int packed_ranges_size, int unpacked_ranges_size, int index)
{
    ...
    log_assert(!wire_node->dimensions.empty());
    ...
    // dimensions contains ranges from wiretype too, but
    ...
    if (wire_node->attributes.count(ID::wiretype)) {
        ...
        if (wiretype_node->type == AST::AST_STRUCT) {
            ...
            skip = (wire_node->dimensions.size()) - id->children.size();
        } else {
    ...
    for (auto i = packed_ranges.size(); i < packed_ranges.size() + unpacked_ranges.size(); i++) {
        range_offset.push_back(wire_node->dimensions[i].range_right);
    }
    // then packed
...
static void convert_packed_unpacked_range(AST::AstNode *wire_node)
{
    ...
    if (convert_node) {
        ...
        if (wire_node->dimensions.empty()) {
            const auto [packed_size, unpacked_size] = set_dimensions(wire_node, packed_ranges, unpacked_ranges);
        ...
        if (!current_node->children.empty() && !current_node->basic_prep) {
            ...
            for (auto child : current_node->children)
                if (child->type == AST::AST_RANGE)
                    ranges_count++;
            if (ranges_count > 1) {
                vector<AST::AstNode *> others = {new AST::AstNode(AST::AST_MULTIRANGE)};
                for (auto child : current_node->children) {
                    if (child->type == AST::AST_RANGE) {
                        others[0]->children.push_back(child);
                    } else {
                        others.push_back(child);
                    }
                }
                std::swap(others, current_node->children);
                others.clear();
            }
        }
        break;
...
void UhdmAst::process_module()
{
    ...
    visit_one_to_many({vpiParameter}, obj_h, [&](AST::AstNode *node) {
        auto attr = get_attribute(node, attr_id::is_imported);
        if (attr) {
            // parameters from packages shouldn't be taken into the account when renaming module
            delete node;
            return;
        }
        attr = get_attribute(node, attr_id::is_type_parameter);
        if (!attr) {
    ...
    visit_one_to_many({vpiParamAssign}, obj_h, [&](AST::AstNode *node) {
        ...
        auto attr = get_attribute(node, attr_id::is_imported);
        if (!attr && node->type != AST::AST_LOCALPARAM) {
            // parameters from packages and local parameters shouldn't be taken into the account when renaming module
...
void UhdmAst::process_param_assign()
{
    ...
    if (find_ancestor({AST::AST_PACKAGE}) != nullptr) {
        set_attribute(current_node, attr_id::is_imported, AST::AstNode::mkconst_int(1, true));
    }
}
